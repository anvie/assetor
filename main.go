...
var Version = "0.1.11" // Autogenerated, don't edit
...
func uploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("in uploadHandler")

	// Check authentication using X-SECRET-KEY header
	secretKey := os.Getenv("SECRET_KEY")
	if secretKey == "" {
		log.Printf("Upload request rejected: Unauthorized - no SECRET_KEY")
	...
	if providedKey != secretKey {
		log.Printf("Upload request rejected: Unauthorized - no X-SECRET-KEY")
		http.Error(w, "Unauthorized", http.StatusUnauthorized)
		return
	}

	log.Println("after authentication checker.")

	const maxUpload = 95 << 20 // 95 MiB (safe under Cloudflare 100MB edge)
	// Hard cap total body size. If client exceeds this, Go returns 413 and stops reading.
	r.Body = http.MaxBytesReader(w, r.Body, maxUpload)

	// Keep memory small; the rest goes to disk (TMPDIR). 32 MiB is fine.
	...
	if strings.Contains(fileName, "..") || !strings.HasPrefix(filepath.Clean(filePath), filepath.Clean("downloads")+string(os.PathSeparator)) {
...
func main() {
	...
	srv := &http.Server{
		Addr:         port,
		Handler:      nil,
		ReadTimeout:  5 * time.Minute, // allow slow uploads
